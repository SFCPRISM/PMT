...
Class Action {
	...
	function save_comment(){
		...
		if(empty($id)){
            ...
            $save = "INSERT INTO comment_list set $data";
			 if ($this->db->query($save) === TRUE) {
            // Get the newly created ID
         $newlyCreatedId = $this->db->insert_id;

        $query = "SELECT CONCAT(users.firstname, ' ', users.lastname) AS commented_by, task_list.*, comments_to_email.*
                  FROM comments_to_email
                  JOIN users ON comments_to_email.user_id = users.id
                  JOIN task_list ON comments_to_email.task_id = task_list.id
                  WHERE comments_to_email.comment_id = " . $newlyCreatedId;
        
        $data = $this->db->query($query)->fetch_assoc();
        
        // If $data is empty, modify the query to join with the sub_task_list table
        if (empty($data)) {
            $query = "SELECT CONCAT(users.firstname, ' ', users.lastname) AS commented_by, sub_task_list.sub_task AS task, comments_to_email.*
                      FROM comments_to_email
                      JOIN users ON comments_to_email.user_id = users.id
                      JOIN sub_task_list ON comments_to_email.task_id = sub_task_list.id
                      WHERE comments_to_email.comment_id = " . $newlyCreatedId;
            ...
            if(!empty($data)){
                ...
                $subject = "" . $data['task'];
                $txt = "Comments: {$data['comment_text']},\ncommented by: {$data['commented_by']}";
                $headers = "From: pmt@cloudprism.in" . "\r\n" . "CC: manhar.shyaam@cloudprism.in";
	...
	function get_productivity(){
		extract($_POST);
		$query = "SELECT CONCAT(users.firstname, ' ', users.lastname) AS user_name, project_list.name AS Project_Name, task_list.task AS Task_Name, user_productivity.* 
		...
		JOIN task_list ON user_productivity.task_id = task_list.id";
